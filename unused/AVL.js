...
function AVLTreeSelectLeftmost(root) {
  while (root.left) {
    root = root.left;
  }
  return root;
}
function AVLTreeSelectRightmost(root) {
  while (root.right) {
    root = root.right;
  }
  return root;
...
function AVLTreeInsertInternal(key, value, root, tree) {
  if (root == null) return CreateAVLTreeNode(key, value);
...
function AVLTreeAssessBalance(root) {
  switch (AVLTreeLeftHeight(root) - AVLTreeRightHeight(root)) {
    case -2:
      return AVL_TREE_IS_UNBALANCED_TO_THE_RIGHT;
    case -1:
      return AVL_TREE_IS_SLIGHTLY_UNBALANCED_TO_THE_RIGHT;
    case 0:
      return AVL_TREE_IS_BALANCED;
    case 1:
      return AVL_TREE_IS_SLIGHTLY_UNBALANCED_TO_THE_LEFT;
    case 2:
      return AVL_TREE_IS_UNBALANCED_TO_THE_LEFT;
  }
}
...
function AVLTreeLeftHeight(root) {
  return !root.left ? -1 : root.left.height;
}
function AVLTreeRightHeight(root) {
  return !root.right ? -1 : root.right.height;
}
